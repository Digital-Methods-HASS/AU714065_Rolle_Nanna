...
```{r relabel column}
# YOUR CODE
#her skifter jeg navnet på 4. kollonne i dokumentet'Western_Europe' til 'Homicide_per_100K'
names(Western_Europe)[4] <- "Homicide_per_100k"
#Herefter efterprøver jeg om det er lykkedes ( hvilket det er :) 
...
#her laver en graf med punkter, ved at vælge datasættet, hvilken data der skal udgøre x og y aksen. Beder den vise det i en graf med punkter med koden 'geom_point' og så har jeg skiftet farven på punkterne til blå :) 
ggplot(data = Western_Europe, aes (x=Year, y=Homicide_per_100k))+ 
...
```{r visualisering af enkeltes europæiske landes mordrate }

  #... YOUR CODE
  #fuktionen 'facet_wrap' kan man dele datasættet i flere kordinatsystemer. ved at skrive 'facet_wrap(~ Entity)', fortæller jeg funktionen at det er kolonnen 'Entity' den skal plottet ud i forskellige koordinatsystemer. 
#ved at tilføje 'colour=Entity' i aes parantesen, giver jeg landene forskellig farve
#jeg bruger 'geo_line()' for at få en linje istedet for punkter som man ellers får ved at bruge 'geom_point'
ggplot(data = Western_Europe, aes (x=Year, y=Homicide_per_100k,colour=Entity))+
  geom_line()+
  facet_wrap(~ Entity)


#Løsning af : change the facet "layout" to two columns and three rows so that the trends are easier to see in horizontal layout
## ved at tilføje 'ncole=2' og 'nrow=3' i 'Entity' parantes, for jeg får 2 kolonner og 3 rækker
#ved funktionen 'ggtitle' giver jeg plottet en overordnet titel 
ggplot(data = Western_Europe, aes (x=Year, y=Homicide_per_100k,colour=Entity))+
  geom_line()+
  facet_wrap(~ Entity, ncol=2, nrow=3)+
  ggtitle("Mordrate lande i Europa")
...
```{r, Danske regenters regeringstid, eval = TRUE}

# YOUR CODE HERE:
#først henter jeg mit 'Danish King datasæt' ind og samtidig beder jeg den om at se bort fra 'NA'
read_csv("data/dkmon2.csv",na="NA")

#så navngiver jeg datasættet 'dkmon', ved at bruge en assignmentkey
dkmon <- read_csv("data/dkmon2.csv",na="NA")

#så ser jeg lige hvordan datasættet ser ud, for at se om det ser rigtigt ud
glimpse(dkmon)

#så laver jeg en 'duration' kolonne 
dkmonduration <- dkmon %>% 
  mutate(duration=Regerings_Slut - Regerings_Start_, na.rm=TRUE) %>%
  mutate(Regerings_Midt_ = Regerings_Start_ + Regerings_Slut/2)
  

#så ser jeg lige hvordan det ser ud
head(dkmonduration)

#så laver jeg en kolonne med 'duration' og samtidig også en ny kolonne der hedder 'midtyear'/Regerings_Midt_
dkmonformat <- dkmon %>% 
  mutate(duration=Regerings_Slut - Regerings_Start_, na.rm=TRUE) %>% 
...
#så ggplotter jeg 'Regerings_Midt_' på X-aksen og 'duration' på y-aksen. 
# jeg fortæller den at den skal tage udgangspunkt i datasættet 'dkmonformat'
#'geom_smooth()' --> giver den blå linje
#'theme_bw()' ændre baggrunden. 
...
#Svar på: - Provide meaningful labels and a title
# ved at bruge 'xlab' og 'ylab' funktionen kan jeg ændre navnene på de to akser. 
dkmonformat %>%
  ggplot(aes(x=Regerings_Midt_, y=duration))+
  geom_point(colour ="blue")+
  geom_smooth()+
  theme_bw()+
  xlab("årstal")+
  ylab("gennemsnits_regeringstid")+
  ggtitle("Danske_regenter")

#How would you characterize the trend compared to the homicides above?
#SVAR: Vi ser at før år 1500, er der flere prikker og de er klumpet mere sammen, hvilket går i tråd med at der er var mer ekmap om tronen, og regenter der levede kortere tid ( fx. fordi de blev slået ihjel). Hvorimod efter år 1500, bliver prikkerne spredt mere ud, hvilket betyder at  regenterne lever/og dermed oftes også reagere i længere tid. 
...
```{r Mordrate i Euroæiske lande}
#ved at tilføje funktionen 'theme(legend.position="bottom"), kommer farve-forklaringen ned i buden, istedet for at vær ei siden. 
#ved at bruge funktionen 'labs(colour='Country') --> beskriver jeg hvad farverne indikere
ggplot(data = Western_Europe, aes (x=Year, y=Homicide_per_100k,colour=Entity))+
  geom_line()+
  facet_wrap(~ Entity, ncol=2, nrow=3)+
  ggtitle("Mordrate lande i Europa")+
