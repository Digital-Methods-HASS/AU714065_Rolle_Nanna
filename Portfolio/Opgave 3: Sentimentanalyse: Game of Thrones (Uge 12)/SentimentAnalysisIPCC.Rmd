---
title: 'Text mining, sentiment analysis, and visualization: Game of Thrones '
...
note: /n betyder en ny linje. Hver gang man arbejdrer med en PDF skal man først tjekke hvordan pdfén bliver transfert til r
```{r single-page}
got_p9 <- got_text[9]
got_p9
...
```{r, del siden op i individuelle linjer}
#Note: vi laver det til en data.frame så det bliver til en kolonne. så bruer vi 'mutat' til at lave en ny kolonne (hvor hver linje bliver en række i denne kolonne), og vi bruger /n til at indikere hvor linjerne skal dele. 
got_df <- data.frame(got_text) %>% 
  mutate(text_full = str_split(got_text, pattern = '\n')) %>% 
  unnest(text_full) %>% 
  mutate(text_full = str_trim(text_full)) 

got_df#her ser vi at der er 2 kolonner men 30.233 rækker, altså svarende til 30.233 linjer. 
...
```{r, individuelle ord eller tokenize}
...
```
Note: Nu ser vi at der er kommet 297.814 rækker, istedet for 30.233 rækker, da hver rækker nu ikke længere udgør en linje, men et ord. 
```{r, gennemse dokument}
got_tokens 
...
Note: her ser vi også hvilke ord der skal i stopordslisten
```{r ordtælling og sortering efter hyppigst forekommende og mindst forekommende}
...
### Credits: 
To Adéla Sabotkova for making the tutorial template 
