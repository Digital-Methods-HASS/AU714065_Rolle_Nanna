...
author: Nanna Rolle 
date: 'created on 19 marts 2025, updated `r format(Sys.time(), "%d %B, %Y")`'
...
### Get the IPCC report:
```{r get-document}
frank_path <- here("data","Frankenstein.pdf")
frank_text <- pdf_text(frank_path)
frank_path
frank_text <- pdf_text(frank_path)

#frank_text # ved at kører koden frank_text, ser vi hvordan r læser testen. /n=ny linje
```
...
```{r single-page}
frank_p72 <- frank_text[72]
frank_p72
#/n betyder en ny linje. Hver gang man arbejdrer med en PDF skal man først tjekke hvordan pdfén bliver transfert til r
...
```{r split-lines}
#del siden op i individuelle linjer
#vi laver det til en data.frame så det bliver til en kolonne. så bruer vi mutat til at lave en ny kolonne (hvor hver linje bliver en række i denne kolonne), og vi bruger /n til at indikere hvor linjerne skal dele. 
frank_df <- data.frame(frank_text) %>% 
  mutate(text_full = str_split(frank_text, pattern = '\n')) %>% 
  unnest(text_full) %>% 
  mutate(text_full = str_trim(text_full)) 

frank_df#her ser vi at der er 2 kolonner men 5,330 rækker, altså svarende til 5,330 linjer. 
...
```{r tokenize}
#vi tgaer dataframe og den nye kolonne vi har lavet 'text_full' og laver en ny kolonne kaldet 'word'. Nu er linjerne eksploderet til individuelle ord, i 'word' kolonnen.
frank_tokens <- frank_df %>% 
  unnest_tokens(word, text_full)
frank_tokens #nu ser vi at der er kommet 75,199 rækker, istedet for 5.330 rækker, da hver rækker nu ikke længere udgår en linje, men et ord. 
...
```{r count-words}
#nu tæller vi ordene og sortere dem efter det hyppigst forekommende til mindst hyppigt forekommende (her kan vi også se hvilke ord der skal i stopordslisten)
frank_wc <- frank_tokens %>% 
  count(word) %>% 
  arrange(-n)
frank_wc
```
...
```{r stopwords}
#her ser vi hvilke stopord der i forvejen er i r
?stop_words
#vi kan se hvordan det ser ud
stop_words
#hvis man gerne vil fjerne et ord fra stopordslisten, så skal man fjerne en række,filter(words= det ord man vil fjerne)

frank_stop <- frank_tokens %>% 
...
#her tager vi dataframe der er en kolonne, og så siger vi filter de dele der as converted to numerict is NA´s (as.numeric).
#vi får kun de ikke numeriske værdier
frank_no_numeric <- frank_stop %>% 
...
```{r wordcloud-prep}
# Der er 6536 unikke ord 

length(unique(frank_no_numeric$word))

# We probably don't want to include them all in a word cloud. Let's filter to only include the top 100 most frequent?
#head funktionen giver normalt de først 6 rækker, men her siger vi at vi vil have de første 100 rækker
frank_top100 <- frank_no_numeric %>% 
  count(word) %>% 
  arrange(-n) %>% 
  head(100)
...
```{r wordcloud}
#her bruger vi disse 100 rækker/ord til at lave en 'wordcloud'
frank_cloud <- ggplot(data = frank_top100, aes(label = word)) +
  geom_text_wordcloud() +
  theme_minimal()
...
```{r wordcloud-pro}
ggplot(data = frank_top100, aes(label = word, size = n)) +
...
```{r bind-afinn}
frank_afinn <- frank_stop %>% 
  inner_join(get_sentiments("afinn"))
```

Let's find some counts (by sentiment ranking):
```{r count-afinn}
frank_afinn_hist <- frank_afinn %>% 
  count(value)

# Plot them: 
ggplot(data = frank_afinn_hist, aes(x = value, y = n)) +
  geom_col(aes(fill = value)) +
  theme_bw()+
  xlab("Semtimental værdi fra -4 til +4")+
  ylab("Antal Ord")
  ggtitle("Kategorisering af ord i sentimentalværdier")
```

Investigate some of the words in a bit more depth:
```{r afinn-2}
# What are these '2' words?
frank_afinn2 <- frank_afinn %>% 
  filter(value == 2)
...
unique(frank_afinn2$word)

# Count & plot them
frank_afinn2_n <- frank_afinn2 %>% 
  count(word, sort = TRUE) %>% 
  mutate(word = fct_reorder(factor(word), n))


ggplot(data = frank_afinn2_n, aes(x = word, y = n)) +
  geom_col() +
  coord_flip() +
  theme_bw()
...
```{r check-exclusions}
frank_exclude <- frank_nrc %>% 
  anti_join(get_sentiments("nrc"))

view(frank_exclude)

# Count to find the most excluded:
frank_exclude_n <- frank_exclude %>% 
  count(word, sort = TRUE)

head(frank_exclude_n, n=20)
```
...
```{r count-bing}
frank_nrc_n <- frank_nrc %>% 
  count(sentiment, sort = TRUE)

# And plot them:

ggplot(data = frank_nrc_n, aes(x = sentiment, y = n)) +
  geom_col(aes(fill = sentiment))+
  theme_bw()+
  ylab("antal ord")+
  xlab("Følelse")+
  ggtitle("Antal ord fordelt på følelse")
```

Or count by sentiment *and* word, then facet:
```{r count-nrc}
frank_nrc_n5 <- frank_nrc %>% 
  count(word,sentiment, sort = TRUE) %>% 
  group_by(sentiment) %>% 
  top_n(5) %>% 
  ungroup()

frank_nrc_gg <- ggplot(data = frank_nrc_n5, aes(x = reorder(word,n), y = n, fill = sentiment)) +
...
# Save it
ggsave(plot = frank_nrc_gg, 
       here("figures","frank_nrc_sentiment.png"), 
       height = 8, 
       width = 5)
...
```{r nrc-confidence}
death <- get_sentiments(lexicon = "nrc") %>% 
  filter(word == "death")
